...
public interface AppointmentRepository extends JpaRepository<Appointment, Integer> {
  ...
  @Query("SELECT e FROM #{#entityName} e "
      + "WHERE (:appointmentStart = NULL OR e.appointmentDate BETWEEN :appointmentStart AND :appointmentEnd) "
      + "AND (e.organizer.firstname LIKE %:organizer% OR e.organizer.lastname LIKE %:organizer%) ")
  ...
  default Page<Appointment> findAll(Pageable pageable,
    ...
    if (appointmentDate != null) {
      ...
      end = LocalDateTime.from(appointmentDate).toLocalDate().plusDays(1L).atStartOfDay().minusSeconds(1L);
    }
